...
Clone the repository: git clone https://github.com/YOUR_USERNAME/oil-price-change-point-analysis.git cd oil-price-change-point-analysis Create and activate a Python virtual environment (recommended): python -m venv venv source venv/bin/activate # On Windows use venv\Scripts\activate Install required dependencies: pip install -r requirements.txt
Exploratory Data Analysis: Run notebooks in notebooks/ folder to visualize trends and prepare data.

Project Structure oil-price-change-point-analysis/ │ ├── data/ # Raw and processed datasets (ignored by git) ├── notebooks/ # Jupyter notebooks for EDA and modeling ├── src/ # Python scripts for modeling and utilities ├── requirements.txt # Project dependencies ├── README.md # This file └── .gitignore # Git ignore rules (includes /data)
